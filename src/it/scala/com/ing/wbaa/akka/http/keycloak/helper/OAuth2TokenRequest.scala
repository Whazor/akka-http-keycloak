package com.ing.wbaa.akka.http.keycloak.helper
...
import com.ing.wbaa.akka.http.keycloak.KeycloakSettings

import scala.concurrent.{ExecutionContextExecutor, Future}

case class KeycloakToken(access_token: String)
...
trait OAuth2TokenRequest {
  ...
  private implicit val keycloakTokenJson: RootJsonFormat[KeycloakToken] = jsonFormat1(KeycloakToken)
  ...
  def keycloakToken(formData: Map[String, String]): Future[KeycloakToken] =
    getTokenResponse(formData).map(_.entity.dataBytes.map(_.utf8String)
      .map(_.parseJson.convertTo[KeycloakToken])
      .runWith(Sink.seq)).flatMap(_.map(_.head)).recover{case _ => KeycloakToken("invalid")}
